...
class ApiService {
  ...
  Future<List<Station>> fetchStationsByVille(String ville) async {
    final stationsResponse = await http.get(Uri.parse('$_baseUrl/stations/by-ville/$ville'));
    if (stationsResponse.statusCode == 200) {
      List<dynamic> stationsJson = json.decode(stationsResponse.body);
      List<Station> stations = [];
      for (var stationJson in stationsJson) {
      ...
      throw Exception('Failed to load stations for ville $ville from API');
    }
  }
